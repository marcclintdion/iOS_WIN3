...
pixelInput bump_vs(vertexInput IN) 
{
    ...
    OUT.hpos = mul(IN.pos, matWVP);         // Vertex in clip space  
    OUT.uv0 = IN.uv0 * tile;                // Copy texcoord
    float3 vertexPos = mul(IN.pos, matW);	// World vertex
    OUT.normalVec = mul(IN.normal, matW);	// Normal vector (Used by Texture Mapping and POM)
    float3x3 tangentMap = float3x3(IN.tangent, IN.binormal, IN.normal);   // Tangent space (Tangent, binormal, normal)
    tangentMap = mul(tangentMap, matW);
    float3 eyeVec = vertexPos - matVI[3].xyz;		// View vector
	OUT.eyeVec = mul(tangentMap, eyeVec);
    float3 lightVec = lightPos - vertexPos;       // Light vector
...
float4 relief_ps(pixelInput IN) : COLOR
{
    ...
    v.xy *= depth;   
    //---------------------------------------------------------------------
    v /= v.z * 8;               // Discretize the space
	
    for(int i = 0; i < 8; i++ ) // Linear serach
	{
		float4 tex = tex2D(cone_map, p.xy);
		if (p.z < tex.w)
			p+=v;
	}
    for(int i = 0; i < 4; i++)  // Binary seracg
	{
		v *= 0.5;
		float4 tex = tex2D(cone_map, p.xy);
		if (p.z < tex.w)
    ...
    //---------------------------------------------------------------------
    float3 color = tex2D(color_map, p).xyz;

    float3 p2 = tex2D(cone_map, p);	    // Bump map
	p2.xy = p2.xy * 2.0 - 1.0;
	p2.y = -p2.y;
    p2.z = sqrt(1.0 - dot(p2.xy, p2.xy));
    //---------------------------------------------------------------------	
    
	float diffuseInt = saturate(dot(IN.lightVec, p2));
	
	float4 finalColor;
	finalColor.xyz = color * diffuseInt * float3(0.8, 0.8, 0.8);
	finalColor.w = 1.0f;
