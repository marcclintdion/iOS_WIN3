...
struct ShaderDescription
{
	...
	const ShaderDescription &operator=(const ShaderDescription &s) { return s; }
...
bool PointInBoundingBox(PVRTVec2 p, PVRTBoundingBox2D bbox)
{
...
{
	static unsigned int initState = 0;
	finished = false;
	...
	if (initState == 0)
	{
		...
		m_Print3D.DisplayDefaultTitle("3D Navigation", NULL, ePVRTPrint3DLogoIMG);
	...
	{
		static unsigned int textureBase = 0;
		...
		{
			// Loaded all textures
			if (textureBase >= m_uiNumTextures)
			{
				initState++;
				break;
			}
			strcpy(buffer, c_paszTextures[textureBase]);
			strcat(buffer, ".pvr");

			if (PVR_SUCCESS == PVRTTextureLoadFromPVR(buffer, &m_pauiTextureIds[textureBase]))
			{
		...
		m_Print3D.DisplayDefaultTitle("3D Navigation", NULL, ePVRTPrint3DLogoIMG);
		m_Print3D.Print3D(35.0f, 45.0f, 1.0f, 0xFFFFFFFF, "PLEASE WAIT");
		m_Print3D.Print3D(35.0f, 55.0f, 0.5f, 0xAAFFFFFF, "Loading textures ... (%d / %d)", textureBase, m_uiNumTextures);
		m_Print3D.Flush();
		return true;
	}
	...
	{
		static unsigned int blockBase = 0;

		if (blockBase >= m_uiNumPVRTCityBlocks)
		{
			UpdateShadowVolumes();
			initState++;
		}
		...
		{
			for (unsigned int j=0; j < m_paPVRTCityBlocks[blockBase].numLod; j++)
			{
				CreateModelVbo(blockBase, j);
			}

			CreateShadowVolumes(blockBase);
			...
			m_Print3D.Print3D(35.0f, 59.0f, 0.5f, 0xAAFFFFFF, "Loading models ... (%d / %d)", blockBase, m_uiNumPVRTCityBlocks);
			m_Print3D.Flush();
			return true;
		}
	}
...
bool OGLES2Navigation3D::Load3dModelIndex(const char *pszFilename, CPVRTString* pErrorStr)
{
	...
	for (unsigned int i=0; i < m_uiNumPVRTCityBlocks; i++)
	{
		...
		memcpy((char *)&m_paPVRTCityBlocks[i].numLod, pData, sizeof(size_t));
		pData += sizeof(size_t);
		...
		for (unsigned int j=0; j < m_paPVRTCityBlocks[i].numLod; j++)
		{
			...
			memcpy((char *)&m_paPVRTCityBlocks[i].paLod[j].numObjects, pData, sizeof(size_t));
			pData += sizeof(size_t);
			...
			for (unsigned int k=0; k < m_paPVRTCityBlocks[i].paLod[j].numObjects; k++)
			{
				...
				pData += sizeof(PVRTBoundingBox2D);
				memcpy((char *)&m_paPVRTCityBlocks[i].paLod[j].paObjects[k].numSubObjects, pData, sizeof(size_t));
				pData += sizeof(size_t);
