...
void triangulate_intersection(const PVRTCoordinateVector &coordinates, Intersection &intersection, vector<PVRTLinestripExt> &linestrips)
{
	...
	for (unsigned int i=0; i < hole_fillers.size(); i++)
	{
		...
		PVRTVertex v0 = *cap.v0;
		PVRTVertex v1 = *cap.v1;
		v0.texcoord.x = -0.125f;
		v1.texcoord.x = 0.625f;
		PVRTVertex v01;
		v01.position = (v0.position + v1.position) * 0.5f;
		v01.texcoord = (v0.texcoord + v1.texcoord) * 0.5f;
		float seg_length = (v1.position - v0.position).length();
		float seg_length_a = (v0.position - v01.position).length();
		float seg_length_b = (v1.position - v01.position).length();
		PVRTVertex midpoint_tri_a = midpoint;
		PVRTVertex midpoint_tri_b = midpoint;

		float texoffset_a = (1.0f * seg_length_a) / seg_length;
		float texoffset_b = (1.0f * seg_length_a) / seg_length;

		midpoint_tri_a.texcoord.x = v0.texcoord.x + texoffset_a;
		midpoint_tri_b.texcoord.x = v1.texcoord.x - texoffset_a;
		
		//midpoint_local.texcoord = PVRTVec2(0.25f, 0.0f);

		intersection.cap_coordinates.push_back(midpoint_tri_a);
		intersection.cap_coordinates.push_back(midpoint_tri_b);
		intersection.cap_coordinates.push_back(v0);
		intersection.cap_coordinates.push_back(v01);
		intersection.cap_coordinates.push_back(v1);
		PVRTTriangle tri_a = { index, index + 2, index + 3 };
		PVRTTriangle tri_b = { index + 3, index + 4, index + 1 };
		intersection.cap_triangles.push_back(tri_a);		
		intersection.cap_triangles.push_back(tri_b);		
/*
		intersection.cap_coordinates.push_back(midpoint_local);
		intersection.cap_coordinates.push_back(v0);
		intersection.cap_coordinates.push_back(v1);
		//PVRTTriangle tri = { midpoint_index, index, index + 1 };
		PVRTTriangle tri = { index, index + 1, index + 2 };
		intersection.cap_triangles.push_back(tri);		
*/
		PVRTVec3 colour(1.0f, 0.0f, 0.0f);
		if (i == 1) colour = PVRTVec3(0.0f, 1.0f, 0.0f);
		else if (i == 2) colour = PVRTVec3(0.0f, 0.0f, 1.0f);
		debug_generate_arrow(colour, v0.position, v01.position, 10.0f, debuglines);
		debug_generate_arrow(colour, v01.position, v1.position, 10.0f, debuglines);
		/*
		debuglines.push_back(cap.v0->position);
		debuglines.push_back(colour);
		debuglines.push_back(cap.v1->position);
		debuglines.push_back(colour);
		*/
		debuglines.push_back(cap.v0->position);
		debuglines.push_back(colour);
		debuglines.push_back(midpoint.position);
		debuglines.push_back(colour * 0.5f);
		debuglines.push_back(cap.v1->position);
		debuglines.push_back(colour);
		debuglines.push_back(midpoint.position);
		debuglines.push_back(colour * 0.5f);
...
/*!*************************************************************************************************
 @Function		merge_dual_intersections
 @Input			linestrips          The vector of linestrips. 
 @Output		intersections       The vector containing all intersections.
 @Description	Builds a map of all intersections.
***************************************************************************************************/
...
void merge_dual_intersections(const PVRTLinestripVector &linestrips, const vector<Intersection> &intersections, PVRTLinestripVector &merged_linestrips)
{
	PVRTLinestripVector tmp_linestrips = linestrips;
	map<unsigned int, unsigned int> index_mapping;
	...
	for (unsigned int i=0; i < intersections.size(); i++)
	{
		...
		if (intersections[i].linestrips.size() == 2)
		{			
			unsigned int mapping_a = index_mapping[intersection.linestrips.front()];
			unsigned int mapping_b = index_mapping[intersection.linestrips.back()];
			const PVRTLinestrip &a = tmp_linestrips[mapping_a];
			const PVRTLinestrip &b = tmp_linestrips[mapping_b];

			PVRTLinestrip merged;
			merged.indices.resize(a.indices.size() + b.indices.size());
			...
			if (a.ref_in_id == intersection.id)
			{
				reverse_copy(a.indices.begin(), a.indices.end(), merged.indices.begin());
				merged.ref_in_id = a.nref_in_id;
			}
			else
			{
				copy(a.indices.begin(), a.indices.end(), merged.indices.begin());
				merged.ref_in_id = a.ref_in_id;
			}

			if (b.ref_in_id == intersection.id)
			{
				copy(b.indices.begin(), b.indices.end(), merged.indices.begin() + a.indices.size());
				merged.nref_in_id = b.nref_in_id;
			}
			else
			{
				reverse_copy(b.indices.begin(), b.indices.end(), merged.indices.begin() + a.indices.size());
				merged.nref_in_id = b.ref_in_id;
			}
			
			tmp_linestrips.push_back(merged);
			index_mapping[intersection.linestrips.front()] = tmp_linestrips.size() - 1;
			index_mapping[intersection.linestrips.back()] = tmp_linestrips.size() - 1;			
		}		
	}
	...
	set<unsigned int>::const_iterator set_iter = valid_linestrips.begin();
	for (; set_iter != valid_linestrips.end(); set_iter++)
		merged_linestrips.push_back(tmp_linestrips[*set_iter]);
}
